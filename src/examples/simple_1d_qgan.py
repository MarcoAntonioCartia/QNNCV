"""
Simple 1D QGAN - Using Existing Modular Components

This script implements a simple 1D quantum generator using the existing
modular components and proper gradient-preserving approach.
"""
...
class Simple1DQuantumGenerator:
    """
    Simple 1D quantum generator using existing modular components.
    
    Uses the proven gradient-preserving approach from your existing implementations.
    """
    
    def __init__(self, latent_dim: int = 4, cutoff_dim: int = 6):
        """
        Initialize simple 1D quantum generator.
        ...
        # Use existing PureSFGenerator with 1D output
        self.quantum_generator = PureSFGenerator(
            latent_dim=latent_dim,
            output_dim=1,  # 1D output
            n_modes=2,     # 2 modes for expressiveness
            layers=2,      # 2 layers for learning capacity
            cutoff_dim=cutoff_dim
        )
        
        print(f"🔧 Simple1DQuantumGenerator initialized:")
        print(f"   Architecture: {latent_dim}D → 2 qumodes → 1D")
        print(f"   Using existing PureSFGenerator component")
        print(f"   Parameters: {self.quantum_generator.get_parameter_count()}")
    
    def generate(self, z: tf.Tensor) -> tf.Tensor:
        """
        Generate 1D samples using existing modular component.
        ...
        """
        # Use the existing generator's generate method
        # This preserves all gradient flow as it's already implemented correctly
        return self.quantum_generator.generate(z)
    
    @property
    def trainable_variables(self) -> list:
        """Get all trainable variables from the quantum generator."""
        return self.quantum_generator.trainable_variables
...
class Simple1DQGANTrainer:
    """Simple trainer for 1D QGAN using existing components."""
    
    def __init__(self, generator: Simple1DQuantumGenerator, discriminator: SimpleClassicalDiscriminator):
    ...
    def train_step(self, real_data: tf.Tensor, batch_size: int = 32):
        """Single training step using existing components."""
        # Generate noise
    ...
    def train(self, real_data: np.ndarray, epochs: int = 50, batch_size: int = 32, 
        ...
        """Train the QGAN using existing components."""
        print(f"🚀 Starting Simple 1D QGAN training for {epochs} epochs...")
    ...
    def save_results(self, real_data: np.ndarray, fake_data: np.ndarray, 
        ...
        os.makedirs("results/simple_1d_qgan", exist_ok=True)
        ...
        # Save plot
        save_path = f"results/simple_1d_qgan/epoch_{epoch:03d}_results.png"
...
def main():
    """Main training function using existing modular components."""
    with clean_training_output():
        print("=" * 80)
        print("🚀 SIMPLE 1D QUANTUM GAN TRAINING")
        print("   Using existing modular components")
        print("=" * 80)
        ...
        for dist_name in distributions:
            ...
            # Create models using existing components
            generator = Simple1DQuantumGenerator(latent_dim=4, cutoff_dim=6)
