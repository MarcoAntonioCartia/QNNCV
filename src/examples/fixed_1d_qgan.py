"""
Fixed 1D QGAN - Addressing Gradient Flow Issues

This script implements a working 1D quantum generator that fixes the gradient flow
issues and uses proper warning suppression.
"""
...
class Fixed1DQuantumGenerator:
    """
    Fixed 1D quantum generator with proper gradient flow.
    
    Key fixes:
    - Proper parameter mapping with gradient flow
    - Real-valued measurements only
    - Simplified architecture to avoid complex number issues
    """
    
    def __init__(self, latent_dim: int = 4, cutoff_dim: int = 6):
        """
        Initialize fixed 1D quantum generator.
        ...
        # Use 1 mode to simplify and avoid complex number issues
        self.quantum_circuit = PureSFQuantumCircuit(
            n_modes=1,  # Single mode to simplify
            n_layers=1,  # Single layer to avoid complexity
            cutoff_dim=cutoff_dim,
            circuit_type="basic"
        )
        
        # Simple parameter mapping (direct linear mapping)
        self.param_mapping = tf.keras.Sequential([
            tf.keras.layers.Dense(4, activation='tanh'),
            tf.keras.layers.Dense(2, activation='tanh')  # Only 2 parameters for 1-mode, 1-layer
        ])
        ...
        dummy_measurement = tf.random.normal([1, 1])  # 1 mode = 1 measurement
        _ = self.output_decoder(dummy_measurement)
        
        print(f"🔧 Fixed1DQuantumGenerator initialized:")
    ...
    def generate(self, z: tf.Tensor) -> tf.Tensor:
        """
        Generate 1D samples from latent input with proper gradient flow.
        ...
        quantum_params = self.param_mapping(z)  # [batch_size, 2]
        
        # Process each sample individually
        outputs = []
        for i in range(batch_size):
            # Get quantum parameters for this sample
            sample_params = quantum_params[i:i+1]  # [1, 2]
            ...
            # Extract measurement (real-valued only)
            measurement = self.quantum_circuit.extract_measurements(quantum_state)  # [1]
...
class Fixed1DQGANTrainer:
    """Fixed trainer for 1D QGAN with proper gradient handling."""
    
    def __init__(self, generator: Fixed1DQuantumGenerator, discriminator: SimpleClassicalDiscriminator):
    ...
    def train_step(self, real_data: tf.Tensor, batch_size: int = 16):
        ...
        # Apply gradients (only if they exist)
    ...
    def train(self, real_data: np.ndarray, epochs: int = 50, batch_size: int = 16, 
              save_interval: int = 10):
        """Train the QGAN with clean output."""
        print(f"🚀 Starting Fixed QGAN training for {epochs} epochs...")
    ...
    def save_results(self, real_data: np.ndarray, fake_data: np.ndarray, 
        ...
        os.makedirs("results/fixed_1d_qgan", exist_ok=True)
        ...
        # Save plot
        save_path = f"results/fixed_1d_qgan/epoch_{epoch:03d}_results.png"
...
def main():
    ...
    with clean_training_output():
        print("=" * 80)
        print("🚀 FIXED 1D QUANTUM GAN TRAINING")
        print("=" * 80)
        ...
        for dist_name in distributions:
            ...
            # Create models
            generator = Fixed1DQuantumGenerator(latent_dim=4, cutoff_dim=6)
            discriminator = SimpleClassicalDiscriminator(input_dim=1)
            
            # Create trainer
            trainer = Fixed1DQGANTrainer(generator, discriminator)
